...
func seaPrimNum() ->[Int]{
    var changeArray = [2,3]//定义数组
    var recordIsPrim = 0 //记录判断质数
...
var primNum = seaPrimNum()
primNum.sort() //升序排序
print("1-1000内的质数升序排序为：\(primNum)")
...
print("1-1000内的质数降序排序（方法一）为：\(primNum)")
...
print("1-1000内的质数降序排序（方法二）为：\(primNum)")
...
print("1-1000内的质数降序排序（方法三）为：\(primNum)")

//降序排序方法四
primNum.sort{ (m,n) in
    return m>n
}
print("1-1000内的质数降序排序（方法四）为：\(primNum)")

//降序排序方法五
primNum.sort(by:>)
print("1-1000内的质数降序排序（方法五）为：\(primNum)")
...
//对数组按age排序并输出
recordArray.sort{ return $0.age > $1.age}
for traverseArray in recordArray{
    print(traverseArray)
}
//对数组按fullName排序并输出
